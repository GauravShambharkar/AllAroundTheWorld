...
export async function getSpotifyAppToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        "Failed to fetch Spotify app access token:",
        await res.text(),
      );
      return null;
    }
...
export const GENRE_ARTIST_POOLS: Record<string, string[]> = {
  ...
  "malayalam sopanam & mappila pattu": [
    "Sai Abhyankkar",
    "Hesham Abdul Wahab",
    "KJ Yesudas",
  ],
  ...
  "tamil gaana & kuthu beats": [
    "Anirudh Ravichander",
    "Gana Bala",
    "Deva Tamil",
    "Santhosh Narayanan",
  ],
  "kollywood tamil cinema music": [
    "Anirudh Ravichander",
    "AR Rahman Tamil",
    "Yuvan Shankar Raja",
    "Ilaiyaraaja",
  ...
  "marathi lavani & powada": [
    "Apsara Aali Lavani",
    "Ajay Atul",
    "Adarsh Shinde",
  ],
  "gujarati garba & dandiya raas": [
    "Falguni Pathak",
    "Aditya Gadhvi",
    "Kinjal Dave",
  ],
  "bengali baul & bhatiali folk": [
    "Anupam Roy",
    "Purna Das Baul",
    "Rupam Islam",
  ],
...
const DISALLOWED_REGIONAL_BLEED = [
  "drake",
  "adele",
  "khalid",
  "taylor swift",
...
export async function searchSpotifyTracks(
  ...
  const matchedGenre = ALL_MICROGENRES.find(
    (g) =>
      ...
      g.id.toLowerCase() === cleanQ ||
      g.name.toLowerCase() === strippedQ,
  );
  ...
  const artistPool =
    GENRE_ARTIST_POOLS[cleanQ] || GENRE_ARTIST_POOLS[strippedQ];
  ...
  searchTermsToTry.push(
    rawQ,
    `${strippedQ} ${matchedGenre?.country || matchedGenre?.subregion || ""}`.trim(),
  );
  ...
  if (isRegional) {
    tracks = tracks.filter(
      (t) =>
        !DISALLOWED_REGIONAL_BLEED.some((bad) =>
          t.artist.toLowerCase().includes(bad),
        ),
    );
  }
    ...
    try {
      ...
      if (iTunesRes.ok) {
        ...
        for (const iTrack of iResults) {
          ...
          if (
            isRegional &&
            DISALLOWED_REGIONAL_BLEED.some((bad) => artistLower.includes(bad))
          ) {
          ...
          if (iTrack.previewUrl) {
            iTunesTracks.push({
              ...
              albumArt:
                iTrack.artworkUrl100?.replace("100x100bb", "300x300bb") || "",
      ...
      console.error(
        `iTunes Audio Preview fetch error for term "${term}":`,
        err,
      );
    }
  }
